...
def _hex_log_path():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(script_dir, "o4_encrypted_hex.log")


def _log_hex(hex_payload):
    """Append encrypted O4 hex to local log (works without license)."""
    try:
        with open(_hex_log_path(), "a") as f:
            f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} {hex_payload}\n")
        print(f"  HEX: {hex_payload[:64]}{'...' if len(hex_payload) > 64 else ''}")
    except OSError as e:
        print(f"  Hex log error: {e}")
